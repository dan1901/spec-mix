...
이 포크는 네 가지 주요 기능 세트를 추가합니다:

1. **모드 시스템** - 일반/프로 워크플로 모드
2. **다국어 지원 (i18n)** - 완전한 국제화 지원
3. **미션 시스템** - 도메인별 워크플로 템플릿
4. **웹 대시보드** - 시각적 프로젝트 관리 인터페이스

## 모드 시스템

Spec Mix는 사용자의 필요와 경험 수준에 맞는 두 가지 운영 모드를 제공합니다.

### 일반 모드 (기본값)

간소화된 명령어를 사용한 가이드 워크플로:

- **자동 명확화**: `/spec-mix.specify`가 사양을 생성한 후 자동으로 명확화 질문을 제시
- **사용자 선택**: 질문에 답하여 사양을 개선하거나 다음 단계로 건너뛰기
- **단계별 작업**: `/spec-mix.plan`이 체크리스트 + 계획 + 단계 수준 작업 생성 (상세 하위 작업 아님)
- **가이드 구현**: `/spec-mix.implement`가 단계별로 실행하며 워크스루와 리뷰 제공
- **승인 워크플로**: 각 단계 후 사용자가 승인/거부 선택 (명령어가 아님)

**일반 모드 워크플로:**

```text
/spec-mix.specify "기능 설명"
    ↓
사양 생성 → 자동 명확화 질문
    ↓
[질문에 답하기] 또는 [건너뛰기 → 다음 단계]
    ↓
/spec-mix.plan
    ↓
체크리스트 + 계획 + 단계별 작업
...
대시보드는 현재 모드에 맞게 적응합니다:

- **일반 모드**: 단계 진행률이 있는 단계 기반 칸반 보드 표시
- **프로 모드**: 레인 관리가 있는 전통적인 워크 패키지 칸반 표시
- 기능 카드에 모드 배지 표시
