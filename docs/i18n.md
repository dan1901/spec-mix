...
### `specify lang install <code>` (Coming Soon)
...
When you run a command like `/speckit.specify`:
...
```bash
SPECIFY_LANG={lang_code} specify init test-project
```
...
The `LocaleManager` class (`src/specify_cli/i18n.py`) handles:
...
```python
from specify_cli.i18n import get_locale_manager, t, init_i18n
...
2. **Check available languages**: Run `specify lang list` to see what's available
